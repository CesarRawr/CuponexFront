...
public class CrearController implements Initializable {
    ...
    private ComboBox<Categoria> statusSelector;

    private EmpresasController userController;
    ...
    public void initData(EmpresasController userController) {
    ...
    private void createHandler(ActionEvent event) {
        ...
        try {
            ...
            String parametros = "nombre="+nombre.getText()+
                                ...
                                "&idEstatusE="+statusSelector.getValue().getIdCatalogo();
    ...
    private void loadStatus() {
        String urlWS = Constante.URL_BASE + "empresa/status";
        
        try{
            String resultadoWS = ConexionServiciosWeb.peticionServicioGET(urlWS);
            Gson gson = new Gson();
            Type typeList = new TypeToken< ArrayList<Categoria> >(){}.getType();
            ArrayList<Categoria> data = gson.fromJson(resultadoWS, typeList);
            
            statusSelector.setButtonCell(cellFactory.call(null));
            statusSelector.setCellFactory(cellFactory);
            statusSelector.getItems().setAll(data);
        }catch (Exception e){
            Utilidades.mostrarAlertaSimple(
                "Error de conexion", 
                "Por el momento no se puedo cargar la lista", 
